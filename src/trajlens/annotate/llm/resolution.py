...
- ADVISORY task (question / explanation / how-to / code-review / recommendation / "is this correct?" / "what's the bug?"): the deliverable IS the answer. The agent finishes by giving a complete, direct answer. CONTAINING CODE DOES NOT MAKE IT AN ACTION TASK — if the user asked a question and the agent answers it (even with a code snippet, a fix, or an example), that is advisory: the agent is telling the user what to do, not changing the user's files. The agent CANNOT run anything in the user's environment, so "the user still has to apply it" is the NORMAL, EXPECTED end — it is NOT incomplete work. A coherent, on-point final answer = resolved. Do NOT downgrade an advisory task to partially_resolved merely because the user did not come back to confirm, because the agent could not execute the suggestion, or because the agent closed with a polite offer of further help ("Would you like me to…", "Let me know if…", "I can also explain…") — that offer is courtesy, NOT unfinished work.
...
- CRITICAL — do not over-credit GENUINELY in-progress work. If the final assistant turn announces it is ABOUT TO DO MORE WORK ITSELF ("next, I'll run…", "now let me edit…", "let me continue implementing…"), or the session ends on a pending tool call / tool output with no concluding wrap-up, the task is NOT finished → "partially_resolved" or "indeterminate", NEVER "resolved". BUT distinguish this from a COMPLETED answer that merely offers optional follow-up ("Would you like me to explain X?", "Let me know if you need Y"): that is a finished advisory answer, not in-progress work — judge it on the answer already given, not on the offer.
